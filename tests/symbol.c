...
void test_symbol_same_name(void) {
  ...
  TEST_CHECK(symbol_lookup(&symbol_table, "x") == NULL);
  symbol_t *result         = symbol_add(&symbol_table, SYM_UNKNOWN, "x", false);
  ...
  TEST_CHECK(symbol_lookup(&symbol_table, "a") == symbol_right);
